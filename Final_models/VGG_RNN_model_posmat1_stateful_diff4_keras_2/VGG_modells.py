...
def VGG_A(keep_prob, batch_size):
    #architecture
    filter_size1 = 3
    num_filters1 = 64
    filter_size2 = 3
    ...
    imput_shape = [1, image_width, image_height, image_depth]

    model = keras.models.Sequential()

    model.add(keras.layers.wrappers.TimeDistributed(keras.layers.convolutional.Conv2D(num_filters1, (filter_size1, filter_size1), strides=(1,1), activation='relu', padding='same'), input_shape=imput_shape, batch_input_shape=(batch_size, 1, 300, 300, 6)))
    ...
    model.add(keras.layers.recurrent.LSTM(256, return_sequences=True, dropout=keep_prob, stateful=True))
