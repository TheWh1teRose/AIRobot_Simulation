...
for modellID in range(3):
	for lr in [1E-3, 1E-4, 1E-5]:
		...
		with graph.as_default():
			...
			if modellID == 1:
				logits = modell.VGG_B(keep_prob, tf_images)
			if modellID == 2:
				logits = modell.VGG_C(keep_prob, tf_images)
		...
		with tf.Session(graph=graph) as session:
			...
			train_writer = tf.summary.FileWriter('statistics/summ_Modell{}_{}'.format(modellID, lr), session.graph)
			total_epochs = 1
			...
			for i in range(epochs):
				...
				if total_epochs % 250 == 0:
					...
					merged = tf.summary.merge_all()
					train_writer.add_summary(merged, total_epochs)
				...
				if total_epochs % 500 == 0:
					save_path = saver.save(session, "ckpts/model_{}.ckpt".format(total_epochs))
			all_acc_test = []
